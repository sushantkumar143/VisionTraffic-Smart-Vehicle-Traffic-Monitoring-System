...
def analyze_report_with_llm(results, speed_threshold):
    ...
    # llm_response = f"""
    # ## AI Assistant Professional Analysis 
    
    # ### 1. Overall Traffic Summary
    # The detection process successfully analyzed a total of **{total_vehicles}** vehicles from the {results.get('source')} input. The model's inherent accuracy is reported at **{results.get('model_accuracy')}**, ensuring high fidelity in classification across all detected objects.
    
    # ### 2. Safety and Compliance Check
    # * **Anomaly Status:** {'**HIGH CONCERN**' if anomaly_count > 0 else '**NORMAL**'}
    # * **High-Speed Events:** **{anomaly_count}** vehicles were flagged for exceeding the user-set safety threshold of **{speed_threshold} km/h**. This ratio of anomalies suggests a traffic enforcement priority area.
    
    # ### 3. Traffic Composition Breakdown
    # The traffic composition is dominated by the **'{most_frequent}'** category, which accounted for {most_frequent_item[1]} instances ({counts_by_type.get(most_frequent) / total_vehicles * 100:.1f}% of traffic). Data indicates {json.dumps(counts_by_type)}.
    
    # ---
    # *Disclaimer: This analysis is based on predictive modeling and simulated LLM output.*
    # """
...
def create_download_report(results, speed_threshold):
    ...
    # Group by Vehicle Type and calculate AVERAGE SPEED
    ...
    if not detailed_log_df.empty and 'Avg_Speed_kmh' in detailed_log_df.columns:
        ...
        df_counts.rename(columns={'index': 'Vehicle_Type'}, inplace=True)
    else:
