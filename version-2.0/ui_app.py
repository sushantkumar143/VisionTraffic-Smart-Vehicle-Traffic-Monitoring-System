    ...
    if not is_video:
        ...
        for i, box in enumerate(res.boxes.data.cpu().numpy()):
            ...
            if is_vehicle:
                ...
                log_rows.append({
                    "UNIQUE_ID": -1, # FIX: Temporary ID for static image/camera input
        ...
        if is_vehicle:
            ...
            if matched_id is not None:
                ...
                tracked_vehicles[matched_id] = {
                    ...
                    'speeds': [],
                    'log_row': { 
                        "UNIQUE_ID": -1, # FIX: Temporary ID for consistent dictionary structure 
...
def display_input_output_comparison(results: dict):
    ...
    if source == 'Video':
        ...
        if os.path.exists(INPUT_FILE_PATH):
             ...
             # Display the input video from its saved location
        ...
        if os.path.exists(OUTPUT_FILE_PATH):
            with open(OUTPUT_FILE_PATH, 'rb') as f:
                video_bytes = f.read()
            # Display the output video from its saved location
...
if analysis_selection == "🏠 Input & Processing":
    ...
    if input_mode == 'Image Upload':
        ...
        if uploaded_file is not None:
            ...
            # Use placeholders for live frame updates
            ...
            try:
                while cap.isOpened():
                    ...
                    # Original frame is needed for input placeholder
                    input_frame_copy = frame_input.copy() 
                    ...
                    last_input_frame = input_frame_copy
                    ...
                    if frame_idx % 5 == 0:
                        # FIX: Display the actual input frame without annotations
                        input_placeholder.image(last_input_frame, channels="BGR", caption="Input Frame (Original)", use_column_width=True)
                ...
                # --- FIX: Re-display playable videos in their final columns ---
                
                # Clear the intermediate image placeholders
                input_placeholder.empty()
                output_placeholder.empty()
                
                # Input Video (Playable)
                if os.path.exists(INPUT_FILE_PATH):
                    with open(INPUT_FILE_PATH, 'rb') as f_in:
                        input_bytes = f_in.read()
                    col_input.header("🎥 Original Input Video (Playable)")
                    col_input.video(input_bytes, format='video/mp4')
                
                # Output Video (Playable)
                if os.path.exists(out_path):
                    with open(out_path, 'rb') as f:
                        video_bytes = f.read()
                    col_output.header("✨ Annotated Output Video (Playable)")
                    col_output.video(video_bytes, format='video/mp4')
